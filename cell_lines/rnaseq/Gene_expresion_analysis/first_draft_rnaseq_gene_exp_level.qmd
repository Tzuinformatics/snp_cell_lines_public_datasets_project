...
# 🧬 2. Load the gene annotation (GTF → tx2gene)
...
# 📁 3. Use your existing metadata (sample_metadata)
...
# 🔗 Join with metadata using Sample column (no need for rownames)
...
# 🧮 Count how many clones per group express each gene > threshold
...
# ✅ Count number of clones per group
group_sizes <- human_coldata %>%
  dplyr::mutate(group = paste(cell_line, genotype, sep = "_")) %>%
  dplyr::count(group, name = "n_clones")

# ✅ Final filter: keep genes expressed in ≥ min_clones_expressed in at least one group
...
```{r}
# 📊 1. Compare expression distribution before vs after filtering
...
# Save FPKM
```{r}
# 🧬 1. Extract gene lengths from GTF (sum of exon widths per gene)
gene_lengths <- gtf_df %>%
  filter(type == "exon") %>%
  group_by(gene_id) %>%
  summarise(gene_length_kb = sum(width) / 1000, .groups = "drop")  # convert to kilobases

# 🧮 2. Get library sizes (total counts per sample in millions)
...
# 🧬 PCA — using your metadata (cell_line, genotype)
...
# 3. Tidybulk pipeline: abundance filter → normalize → PCA
pca_result <- se %>%
...
# 5. Tidybulk pipeline: abundance filter → normalize → PCA
pca_nomt <- se_nomt %>%
  tidybulk::keep_abundant() %>%
  tidybulk::scale_abundance() %>%
  tidybulk::reduce_dimensions(method = "PCA", .dims = 15)
...
# 6. Tidybulk pipeline: abundance filter → normalize → PCA
pca_nomt <- tidy_nomt %>%
  keep_abundant() %>%
  scale_abundance() %>%
...
volcano_plot <- ggplot(volcano_data, aes(
  ...
  labs(
    title = "Volcano Plot: NALM6 TT vs CC (padj ≤ 0.05, FC ≥ 1.5)",
    x = "log2(Fold Change)",
...
## DESEq Loop to run DEG NALM6 ONLY rib0
...
# 🧬 2. Function to run DESeq2 and volcano plot
run_deg_deseq <- function(count_matrix, metadata, ref_genotype, test_genotype, out_prefix) {
  
  # 📂 Filter metadata for NALM6 and selected genotypes
  meta_data <- metadata %>%
  ...
  dds <- DESeq(dds)
  res <- results(dds, contrast = c("genotype", test_genotype, ref_genotype), independentFiltering = TRUE)
...
deg_df <- deg_df %>% filter(!is.na(pvalue), !is.na(log2FoldChange))
  ...
  # 📊 Volcano plot
  x_max <- max(abs(deg_df$log2FoldChange), na.rm = TRUE)
  y_max <- max(-log10(deg_df$pvalue), na.rm = TRUE)
  
  # 🧬 Highlight ARID5B
  highlight_gene <- "ENSG00000150347"  # ARID5B
  highlight_data <- deg_df %>% filter(gene_id == highlight_gene)
  
  volcano_plot <- ggplot(deg_df, aes(x = log2FoldChange, y = -log10(pvalue),
    ...
    # 🔴 Highlight ARID5B point
    geom_point(data = highlight_data, aes(x = log2FoldChange, y = -log10(pvalue)),
               color = "#1F77B4", fill = "white", shape = 21, size = 3) +
    
    # 🏷️ Add offset label with square box + pointer line
    geom_label_repel(
      data = highlight_data,
      aes(x = log2FoldChange, y = -log10(pvalue),
          label = ifelse(!is.na(gene_name), gene_name, gene_id)),
      nudge_x = 2, nudge_y = 2,            # push label away
      color = "black", fill = "white",
      fontface = "bold", size = 5,
      box.padding = 0.5, point.padding = 0.3,
      segment.color = "#1F77B4", segment.size = 1,  # pointer line
  ...
  ggsave(
  filename = file.path(out_dir, paste0(out_prefix, "_volcano.pdf")),
  plot = volcano_plot,
  width = 8, height = 6, units = "in"
)

  
  list(results = deg_df, plot = volcano_plot)
}

# 🚀 3. Run for the 3 desired comparisons
deg_tt_vs_tc <- run_deg_deseq(sf.df.data, sample_metadata, "TC", "TT", "DEG_gen_exp_NALM6_TT_vs_TC")
deg_tc_vs_cc <- run_deg_deseq(sf.df.data, sample_metadata, "CC", "TC", "DEG_gen_exp_NALM6_TC_vs_CC")
deg_tt_vs_cc <- run_deg_deseq(sf.df.data, sample_metadata, "CC", "TT", "DEG_gen_exp_NALM6_TT_vs_CC")
...
## TPM Loop to run DEG NALM6 ONLY 
...
# 📂 1. Create output folder
tpm_out_dir <- "TPM_deg_results_NALM6"
if (!dir.exists(tpm_out_dir)) dir.create(tpm_out_dir)

# 🧮 Helper: convert counts → TPM
...
# 🧬 2. Function to run limma-trend DEG on TPM
run_deg_tpm <- function(count_matrix, metadata, ref_genotype, test_genotype, out_prefix) {
  ...
  # 💾 Save results
  write.csv(tpm_res, file = file.path(tpm_out_dir, paste0(out_prefix, ".csv")), row.names = FALSE)
  
  # === Volcano plot ===
  plot_df <- tpm_res %>% filter(is.finite(logFC), is.finite(P.Value))
   x_max <- max(abs(tpm_res$logFC), na.rm = TRUE)
  y_max <- max(-log10(tpm_res$P.Value), na.rm = TRUE)
  
  # 🧬 Highlight ARID5B
  highlight_gene <- "ENSG00000150347"
  highlight_data <- plot_df %>% filter(gene_id == highlight_gene)
  
  volcano_plot_tpm <- ggplot(plot_df, aes(x = logFC, y = -log10(P.Value),
                                          fill = significance, alpha = significance)) +
    geom_point(shape = 21, colour = "black", size = 2) +
    geom_hline(yintercept = -log10(0.05), linetype = "dashed", colour = "grey50") +
    geom_vline(xintercept = c(-0.2, 0.2), linetype = "dashed", colour = "grey50") +
    ...
    theme_minimal(base_size = 14) +
    labs(
      title = paste0("Volcano: Ribo0 NALM6 ", test_genotype, " vs ", ref_genotype, 
                     " (limma-trend, TPM)"),
      x = "log2 Fold Change", y = "-log10(p-value)", fill = "Significance"
    )
  
  
# 💾 Save volcano plot (PDF for vector quality)
ggsave(
  filename = file.path(tpm_out_dir, paste0(out_prefix, "_volcano.pdf")),
  plot = volcano_plot_tpm,
  width = 8, height = 6, units = "in"
)

  
  list(results = tpm_res, plot = volcano_plot_tpm)
}

# 🚀 3. Run comparisons
tpm_tt_vs_tc <- run_deg_tpm(sf.df.data, sample_metadata, "TC", "TT", "TPM_NALM6_TT_vs_TC")
tpm_tc_vs_cc <- run_deg_tpm(sf.df.data, sample_metadata, "CC", "TC", "TPM_NALM6_TC_vs_CC")
tpm_tt_vs_cc <- run_deg_tpm(sf.df.data, sample_metadata, "CC", "TT", "TPM_NALM6_TT_vs_CC")
...
### Excluding TT Clone 1 and non-coding genes
```{r}
# ==============================================================
# DEG analysis for NALM6 (TT, TC, CC) using limma-trend on TPM
# Ribo0 dataset: Excluding TT Clone 1 and non-coding genes
# ==============================================================
...
# 📂 1. Create output folder
ribo0_tpm_outdir <- "Ribo0_TPM_deg_results_NALM6_filtered"
if (!dir.exists(ribo0_tpm_outdir)) dir.create(ribo0_tpm_outdir)
...
# 🧬 2. Function to run limma-trend DEG on TPM (filtered)
run_deg_tpm_ribo0 <- function(count_matrix, metadata, ref_genotype, test_genotype, out_prefix) {
    
    # ---- Filter metadata (remove TT Clone 1) ----
    meta_data <- metadata %>%
        filter(cell_line == "NALM6",
               genotype %in% c(ref_genotype, test_genotype),
               sample_id_full != "NALM6_TT_1") %>%
        mutate(genotype = factor(genotype, levels = c(ref_genotype, test_genotype)))
    rownames(meta_data) <- meta_data$sample_id_full
    ...
    # 💾 Save results
    write.csv(tpm_res, file = file.path(ribo0_tpm_outdir, paste0(out_prefix, ".csv")), row.names = FALSE)
    ...
    volcano_plot <- ggplot(plot_df, aes(x = logFC, y = -log10(P.Value),
                                        fill = significance, alpha = significance)) +
        geom_point(shape = 21, colour = "black", size = 2) +
        geom_hline(yintercept = -log10(0.05), linetype = "dashed", colour = "grey50") +
        geom_vline(xintercept = c(-0.2, 0.2), linetype = "dashed", colour = "grey50") +
        ...
        theme_minimal(base_size = 14) +
        labs(
            title = paste0("Ribo0 Volcano (coding genes only; removed TT Clone 1): NALM6 ",
                           test_genotype, " vs ", ref_genotype),
            x = "log2 Fold Change", y = "-log10(p-value)", fill = "Significance"
        )
    
    # 💾 Save plot
    ggsave(
        filename = file.path(ribo0_tpm_outdir, paste0(out_prefix, "_volcano.pdf")),
        plot = volcano_plot,
        width = 8, height = 6, units = "in"
    )
...
# 🚀 3. Run comparisons
ribo0_tpm_tt_vs_tc <- run_deg_tpm_ribo0(sf.df.data, sample_metadata, "TC", "TT", "Ribo0_TPM_NALM6_TT_vs_TC")
ribo0_tpm_tc_vs_cc <- run_deg_tpm_ribo0(sf.df.data, sample_metadata, "CC", "TC", "Ribo0_TPM_NALM6_TC_vs_CC")
ribo0_tpm_tt_vs_cc <- run_deg_tpm_ribo0(sf.df.data, sample_metadata, "CC", "TT", "Ribo0_TPM_NALM6_TT_vs_CC")

# 📊 Print plots
ribo0_tpm_tt_vs_tc$plot
ribo0_tpm_tc_vs_cc$plot
ribo0_tpm_tt_vs_cc$plot
...
# 🧮 Compute mean VST expression per genotype
...
dose_response_summary <- vst_long %>%
  ...
  mutate(
    trend_direction = case_when(
      spearman_rho >= 0.5 ~ "↑ (increased with C dosage)",
      spearman_rho <= -0.5 ~ "↓ (decreased with C dosage)",
      TRUE ~ "– (weak/no trend)"
    )
  ) %>%
...
First, identify TT vs CC DEGs using log2FC and p-value cutoffs; then, keep only those with meaningful FPKM in at least one genotype; finally, among those, retain genes where TC shows expression between TT and CC — indicating a graded, dosage-like effect.
...
cat("✅ Genes passing all filters:", nrow(final_genes), "\n")
...
if (length(missing_reh) > 0) warning("❗ Missing samples: ", paste(missing_reh, collapse = ", "))
...
# ✅ Output summary
cat("✅ Genes passing all filters (REH TT vs CC):", nrow(final_genes_reh), "\n")
...
## Loop to run DEG on REH
```{r}
# 📂 1. Create output folder for REH
out_dir_reh <- "deg_results_REH"
if (!dir.exists(out_dir_reh)) dir.create(out_dir_reh)

# 🧬 2. Function to run DESeq2 for REH comparisons
run_deg_reh <- function(count_matrix, metadata, ref_genotype, test_genotype, out_prefix) {
  
  # 📂 Filter metadata for REH and selected genotypes
  meta_data <- metadata %>%
    filter(cell_line == "REH", genotype %in% c(ref_genotype, test_genotype)) %>%
    mutate(genotype = factor(genotype, levels = c(ref_genotype, test_genotype)))
  ...
  deg_df <- as.data.frame(res) %>%
    tibble::rownames_to_column("gene_id") %>%
    left_join(tx2gene_df %>% select(gene_id, gene_name) %>% distinct(gene_id, .keep_all = TRUE), 
              by = "gene_id") %>%
    mutate(significance = case_when(
      pvalue <= 0.05 & log2FoldChange >=  0.2 ~ "up",
      pvalue <= 0.05 & log2FoldChange <= -0.2 ~ "down",
    ...
    left_join(avg_fpkm_grouped, by = "gene_id")
  
  # 💾 Save results
  write.csv(deg_df, file = file.path(out_dir_reh, paste0(out_prefix, ".csv")), row.names = FALSE)
  
  invisible(deg_df)
}

# 🚀 3. Run for REH comparisons
deg_reh_tt_vs_tc <- run_deg_reh(sf.df.data, sample_metadata, "TC", "TT", "DEG_gen_exp_REH_TT_vs_TC")
deg_reh_tc_vs_cc <- run_deg_reh(sf.df.data, sample_metadata, "CC", "TC", "DEG_gen_exp_REH_TC_vs_CC")
deg_reh_tt_vs_cc <- run_deg_reh(sf.df.data, sample_metadata, "CC", "TT", "DEG_gen_exp_REH_TT_vs_CC")
...
```{r}
# ==============================================================
# DEG analysis for REH clones (TT, TC, CC) using limma-trend on TPM
# ==============================================================
...
# 📂 1. Create output folder
reh_tpm_out_dir <- "TPM_deg_results_REH"
if (!dir.exists(reh_tpm_out_dir)) dir.create(reh_tpm_out_dir)
...
# 🧬 2. Function to run limma-trend DEG on TPM
run_deg_tpm_reh <- function(count_matrix, metadata, ref_genotype, test_genotype, out_prefix) {
  ...
  reh_tpm_res <- topTable(fit, coef = coef_name, number = Inf) %>%
    rownames_to_column("gene_id") %>%
    left_join(tx2gene_df %>% distinct(gene_id, gene_name), by = "gene_id") %>%
    mutate(
      significance = case_when(
  ...
  # 💾 Save results
  write.csv(reh_tpm_res, file = file.path(reh_tpm_out_dir, paste0(out_prefix, ".csv")), row.names = FALSE)
  
  # === Volcano plot ===
  plot_df <- reh_tpm_res %>% filter(is.finite(logFC), is.finite(P.Value))
  x_max <- max(abs(reh_tpm_res$logFC), na.rm = TRUE)
  y_max <- max(-log10(reh_tpm_res$P.Value), na.rm = TRUE)
  
  # 🧬 Highlight ARID5B
  highlight_gene <- "ENSG00000150347"
  highlight_data <- plot_df %>% filter(gene_id == highlight_gene)
  
  volcano_plot_reh <- ggplot(plot_df, aes(x = logFC, y = -log10(P.Value),
                                          fill = significance, alpha = significance)) +
    geom_point(shape = 21, colour = "black", size = 2) +
    geom_hline(yintercept = -log10(0.05), linetype = "dashed", colour = "grey50") +
    geom_vline(xintercept = c(-0.2, 0.2), linetype = "dashed", colour = "grey50") +
    ...
    theme_minimal(base_size = 14) +
    labs(
      title = paste0("Volcano: REH ", test_genotype, " vs ", ref_genotype, 
                     " (limma-trend, TPM)"),
      x = "log2 Fold Change", y = "-log10(p-value)", fill = "Significance"
    )
  ...
  # 💾 Save volcano plot
  ggsave(
    filename = file.path(reh_tpm_out_dir, paste0(out_prefix, "_volcano.pdf")),
    plot = volcano_plot_reh,
    width = 8, height = 6, units = "in"
  )
  
  list(results = reh_tpm_res, plot = volcano_plot_reh)
}

# 🚀 3. Run comparisons for REH
reh_tpm_tt_vs_tc <- run_deg_tpm_reh(sf.df.data, sample_metadata, "TC", "TT", "TPM_REH_TT_vs_TC")
reh_tpm_tc_vs_cc <- run_deg_tpm_reh(sf.df.data, sample_metadata, "CC", "TC", "TPM_REH_TC_vs_CC")
reh_tpm_tt_vs_cc <- run_deg_tpm_reh(sf.df.data, sample_metadata, "CC", "TT", "TPM_REH_TT_vs_CC")

# 📊 Print volcano plots
reh_tpm_tt_vs_tc$plot
reh_tpm_tc_vs_cc$plot
reh_tpm_tt_vs_cc$plot


```


## DEG analysis for REH (TT, TC, CC) using limma-trend on TPM
### Excluding TT Clone 1 and non-coding genes
```{r}
# ==============================================================
# DEG analysis for REH (TT, TC, CC) using limma-trend on TPM
# Ribo0 dataset: Keep all clones, remove non-coding genes
# ==============================================================
...
# 📂 1. Create output folder
ribo0_reh_outdir <- "Ribo0_TPM_deg_results_REH_filtered"
if (!dir.exists(ribo0_reh_outdir)) dir.create(ribo0_reh_outdir)
...
run_deg_tpm_reh <- function(count_matrix, metadata, ref_genotype, test_genotype, out_prefix) {
    ...
    # ---- Filter metadata (REH only; keep all clones) ----
    meta_data <- metadata %>%
        filter(cell_line == "REH",
               genotype %in% c(ref_genotype, test_genotype)) %>%
        mutate(genotype = factor(genotype, levels = c(ref_genotype, test_genotype)))
    rownames(meta_data) <- meta_data$sample_id_full
    ...
    # 💾 Save results
    write.csv(tpm_res, file = file.path(ribo0_reh_outdir, paste0(out_prefix, ".csv")), row.names = FALSE)
    ...
    # 🧬 Highlight ARID5B (if present in REH)
    highlight_gene <- "ENSG00000150347"
    highlight_data <- plot_df %>% filter(gene_id == highlight_gene)
    ...
    volcano_plot <- ggplot(plot_df, aes(x = logFC, y = -log10(P.Value),
        ...
        theme_minimal(base_size = 14) +
        labs(
            title = paste0("Ribo0 Volcano (coding genes only): REH ",
                           test_genotype, " vs ", ref_genotype),
            x = "log2 Fold Change", y = "-log10(p-value)", fill = "Significance"
        )
    ...
    # 💾 Save plot
    ggsave(
        filename = file.path(ribo0_reh_outdir, paste0(out_prefix, "_volcano.pdf")),
        plot = volcano_plot,
        width = 8, height = 6, units = "in"
    )
...
# 🚀 3. Run REH comparisons
reh_tpm_tt_vs_tc <- run_deg_tpm_reh(sf.df.data, sample_metadata, "TC", "TT", "Ribo0_TPM_REH_TT_vs_TC")
reh_tpm_tc_vs_cc <- run_deg_tpm_reh(sf.df.data, sample_metadata, "CC", "TC", "Ribo0_TPM_REH_TC_vs_CC")
reh_tpm_tt_vs_cc <- run_deg_tpm_reh(sf.df.data, sample_metadata, "CC", "TT", "Ribo0_TPM_REH_TT_vs_CC")
