...
# 📂 1. Create output folder
nalm6_polyA_outdir <- "PolyA_TPM_deg_results_NALM6_filtered"
...
run_deg_tpm_nalm6_filtered <- function(count_matrix, metadata, ref_genotype, test_genotype, out_prefix) {
    
    # ---- Filter metadata ----
    meta_data <- metadata %>%
        filter(cell_line == "NALM6",
               genotype %in% c(ref_genotype, test_genotype),
               sample_id_full != "NALM6_TT_1") %>%       # remove TT Clone 1
        mutate(genotype = factor(genotype, levels = c(ref_genotype, test_genotype)))
    ...
    volcano_plot <- ggplot(tpm_res, aes(x = logFC, y = -log10(P.Value),
        ...
        theme_minimal(base_size = 14) +
        labs(
            title = paste0("PolyA Volcano (coding genes only; removed TT Clone 1): NALM6 ", test_genotype, " vs ", ref_genotype),
            x = "log2 Fold Change", y = "-log10(p-value)", fill = "Significance"
        )
...
# 🚀 Run filtered comparisons
nalm6_polyA_tt_vs_tc <- run_deg_tpm_nalm6_filtered(sf.df.data, sample_metadata, "TC", "TT", "PolyA_TPM_NALM6_TT_vs_TC")
nalm6_polyA_tc_vs_cc <- run_deg_tpm_nalm6_filtered(sf.df.data, sample_metadata, "CC", "TC", "PolyA_TPM_NALM6_TC_vs_CC")
nalm6_polyA_tt_vs_cc <- run_deg_tpm_nalm6_filtered(sf.df.data, sample_metadata, "CC", "TT", "PolyA_TPM_NALM6_TT_vs_CC")
