...
# Import GTF
gtf <- import("/Users/cojulian/Desktop/Tzu_projects/Matt/reference_genomes/Homo_sapiens.GRCh38.113.gtf.gz")
...
## 🔹 Chunk 2. Align metadata for downstream analysis
...
```{r}
# ==============================================
# 🔹 Compute FPKM and plot ARID5B expression
# ==============================================
...
# ✅ Updated QC Suite (SummarizedExperiment + plots)

```{r}
# ============================================================
# ✅ Updated QC Suite — NALM6 (18 samples, with batches)
# ============================================================
...
pre_comb_arid_plot <- ggplot(arid5b_cpm, aes(x = sample_label, y = CPM,
    ...
    labs(
        ...
        subtitle = "Pre–ComBat-seq | Color = Genotype, dashed outline = Batch2",
...
post_comb_arid_plot <- ggplot(arid5b_cpm_combat, aes(x = sample_label, y = CPM,
    ...
    labs(
        ...
        subtitle = "Post–ComBat-seq | Color = Genotype, dashed outline = Batch2",
        x = "Sample (Genotype_Clone_Replicate)",
        y = "CPM (Counts per Million)",
        fill = "Genotype"
    )
...
```{r}
# ============================================================
# ARID5B (ENSG00000150347) — FPKM before vs after ComBat-seq
...
pre_comb_arid_fpkm_plot <- ggplot(arid5b_pre, aes(x = sample_label, y = FPKM,
    ...
    labs(
        ...
        subtitle = "Pre–ComBat-seq | Fill = Genotype, dashed outline = Batch2",
        x = "Sample (Genotype_Clone_Replicate)",
        y = "FPKM",
        fill = "Genotype"
    )
...
post_comb_arid_fpkm_plot <- ggplot(arid5b_post, aes(x = sample_label, y = FPKM,
    ...
    labs(
        title    = "ARID5B Expression (FPKM)",
        subtitle = "Post–ComBat-seq | Fill = Genotype, dashed outline = Batch2",
        x = "Sample (Genotype_Clone_Replicate)",
        y = "FPKM",
        fill = "Genotype"
    )
...
run_deg_tpm_nalm6 <- function(count_matrix, metadata, ref_genotype, test_genotype, out_prefix) {
  ...
  volcano_plot <- ggplot(tpm_res, aes(x = logFC, y = -log10(P.Value),
    ...
    # 🔵 Highlight ARID5B (only if present)
...
# 🧬 2. Function for limma-trend DEG on TPM
run_deg_tpm_nalm6 <- function(count_matrix, metadata, ref_genotype, test_genotype, out_prefix,
    ...
    volcano_plot <- ggplot(tpm_res, aes(x = logFC, y = -log10(P.Value),
        ...
        labs(
            title = paste0(
                "PolyA Volcano (coding genes only, thresholds: p ≤ ", p_cutoff,
                ", |log₂FC| ≥ ", fc_cutoff, "): NALM6 ", test_genotype, " vs ", ref_genotype
            ),
            x = "log₂ Fold Change",
...
# 🚀 Run pairwise comparisons (include all clones)
...
# NALM6 vs Wenjian (ProB) — TPM, p ≤ 0.05, |log2FC| ≥ 0.5
# ==============================================================
suppressPackageStartupMessages({
    library(dplyr)
    library(readr)
...
# ==============================================================
# 🚀 Run all contrasts (TTvsCC, TCvsCC, TTvsTC)
# ==============================================================
...
intersect_tbl <- map2_dfr(
    ...
    ~{
        p_up <- make_venn_plot(.x$nalm6, .x$wenjian, "up",
                               paste0("Common UP genes (", .y, ", TPM, p ≤ 0.05, |log₂FC| ≥ 0.5)"))
        p_dn <- make_venn_plot(.x$nalm6, .x$wenjian, "down",
                               paste0("Common DOWN genes (", .y, ", TPM, p ≤ 0.05, |log₂FC| ≥ 0.5)"))
...
message("✅ Saved intersection table: ", csv_path)
...
make_venn_pair <- function(nalm6_df, contrast, sig_dir, scenario_label) {
  ggvenn(
    ...
    ggtitle(paste0(
      "Common ", toupper(sig_dir), " genes (", contrast, ", TPM, p ≤ 0.05, |log₂FC| ≥ 0.5)\n",
      scenario_label
    )) +
...
# ==============================================================
# 🚀 Generate Venn diagrams for each scenario
# ==============================================================
...
for (scen_name in names(scenarios)) {
  ...
  # Make 6-panel Venn (3 contrasts × up/down)
...
sample_metadata <- tibble(
    ...
    mutate(
        ...
        replicate = str_extract(sample, "REP[0-9]+") %>% str_remove("REP") %>% as.integer(),
        batch = case_when(
            str_detect(sample, "REP1") ~ "Batch1",
            str_detect(sample, "REP2") ~ "Batch2",
            TRUE ~ "Unknown"
        ),
        sample_id_full = paste(cell_line, genotype, paste0("C", clone), paste0("R", replicate), sep = "_")
    )
...
# ------------------------------------
# 1) GTF: tx2gene and gene metadata
# ------------------------------------
# Robust path selection in case of minor typos in "genomes/genomces"
gtf_candidates <- c(
    "/Users/cojulian/Desktop/Tzu_projects/Matt/reference_genomes/Homo_sapiens.GRCh38.113.gtf.gz",
    "/Users/cojulian/Desktop/Tzu_projects/Matt/reference_genomces/Homo_sapiens.GRCh38.113.gtf.gz"
)
gtf_path <- gtf_candidates[file.exists(gtf_candidates)][1]
if (is.na(gtf_path)) stop("GTF not found at expected paths.")
...
# ------------------------------------
# Initialize from the first file to get gene_id universe
tmp <- read.delim(human_coldata$files[1], check.names = FALSE) %>%
    inner_join(tx2gene_df, by = c("Name" = "transcript_id")) %>%
    group_by(gene_id) %>%
    summarise(NumReads = as.integer(sum(NumReads)), .groups = "drop")

num.row <- nrow(tmp)
num.col <- nrow(human_coldata)

sf.df <- matrix(0L, nrow = num.row, ncol = num.col,
                dimnames = list(tmp$gene_id, human_coldata$Sample))

for (i in seq_along(human_coldata$files)) {
    cat("Reading:", human_coldata$files[i], "\n")
    gi <- read.delim(human_coldata$files[i], check.names = FALSE) %>%
        inner_join(tx2gene_df, by = c("Name" = "transcript_id")) %>%
        group_by(gene_id) %>%
        summarise(NumReads = as.integer(sum(NumReads)), .groups = "drop")
    # Ensure gene_id order aligns to sf.df rows
    sf.df[, i] <- gi$NumReads[match(rownames(sf.df), gi$gene_id)]
}

Salmon.matrix <- as.matrix(sf.df)

cat("RNA raw counts:", nrow(Salmon.matrix), "genes ×", ncol(Salmon.matrix), "samples\n")

# ------------------------------------
# 3) CPM-aware filtering (edgeR)
# ------------------------------------
y   <- DGEList(counts = Salmon.matrix)
grp <- factor(human_coldata$genotype[match(colnames(Salmon.matrix), human_coldata$Sample)],
              levels = c("TT","TC","CC"))

keep <- filterByExpr(y, group = grp)
sf.df.data <- Salmon.matrix[keep, , drop = FALSE]

cat("[filterByExpr] kept", sum(keep), "of", nrow(Salmon.matrix), "genes\n")

# ------------------------------------
# 4) VST (DESeq2)
# ------------------------------------
dds_rna <- DESeqDataSetFromMatrix(
    countData = round(sf.df.data),
    colData   = human_coldata[colnames(sf.df.data), , drop = FALSE],
    design    = ~ genotype + batch   # include batch in design; VST blind=TRUE ignores, but size factors benefit
)
dds_rna <- estimateSizeFactors(dds_rna)
vst_rna <- assay(vst(dds_rna, blind = TRUE))   # genes × samples

# ------------------------------------
# 5) Map to gene_name and aggregate duplicates
# ------------------------------------
rna_df <- as.data.frame(vst_rna) %>%
    tibble::rownames_to_column("gene_id") %>%
    left_join(gene_map, by = "gene_id") %>%
    mutate(gene_name = dplyr::coalesce(gene_name, gene_id)) %>%
    select(-gene_biotype)
...
rna_gene_vst <- rna_df %>%
    ...
    as.matrix()   # genes × samples

cat("RNA gene-level VST:", nrow(rna_gene_vst), "genes ×", ncol(rna_gene_vst), "samples\n")

# ------------------------------------
# 6) Keep NALM6 TT/TC/CC and save CSV (samples × genes)
# ------------------------------------
# Your metadata is already all NALM6, but guard just in case:
nalms_all <- grepl("^NALM6_(TT|TC|CC)_", colnames(rna_gene_vst))
rna_gene_vst <- rna_gene_vst[, nalms_all, drop = FALSE]

rna_for_scca <- t(rna_gene_vst)   # samples × genes




out_csv <- "/Users/cojulian/Desktop/Tzu_projects/Matt/Clarissa/snp_cell_lines_public_datasets_project/cell_lines/rnaseq/PolyA_Rnaseq_nickase/polyA_matrix_NALM6.csv"
write.csv(rna_for_scca, file = out_csv, quote = FALSE)
cat("✅ Saved RNA matrix (samples × genes) to:\n", out_csv, "\n")
