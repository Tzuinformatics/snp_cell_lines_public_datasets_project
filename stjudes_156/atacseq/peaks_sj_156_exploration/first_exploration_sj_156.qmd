...
```{r}
# ================================================================
# CONFUSION MATRIX — Naives removed + Dev order + Stacked bars + Confidence
# ================================================================
suppressPackageStartupMessages({
    library(ggplot2)
    library(FNN)
...
cat("\n=== CONFUSION MATRIX (FULL VERSION) — Naives removed ===\n")

# -------------------------------------------------------------
# 0. Define developmental order
# -------------------------------------------------------------
dev_order <- c("HSC", "CLP", "ProB", "PreB", "Immature-B", "Transitional-B")

# -------------------------------------------------------------
# 1. Healthy reference (remove Naives)
# -------------------------------------------------------------
healthy_ref <- umap_df %>%
    dplyr::filter(cohort == "Healthy", !is.na(cell_type)) %>%
    dplyr::filter(!(cell_type %in% c("Naive-CD5-", "Naive-CD5--")))

if (nrow(healthy_ref) == 0)
    stop("No Healthy samples with valid cell_type.")
...
healthy_centroids <- healthy_ref %>%
    ...
    dplyr::arrange(factor(cell_type, levels = dev_order))

all_lineages <- healthy_centroids$cell_type

cat("[1] Healthy centroids:\n")
...
# Compute ALL distances (for confidence ratio)
...
conf_ratio <- closest / second_closest  # closer to 0 → more confident

sj_samples_umap <- sj_samples_umap %>%
    dplyr::mutate(
        inferred_lineage = healthy_centroids$cell_type[nearest_idx],
...
cat("\n[2] Confusion matrix with developmental ordering:\n")
print(conf_mat)
...
ggplot(conf_df_plot,
    ...
    labs(
        ...
        subtitle = "Nearest-centroid classifier (ComBat-corrected), Naives removed",
        x = "Inferred lineage (Healthy reference)",
        y = "SJ156 subtype"
    )

# -------------------------------------------------------------
# 5. Subtype-specific stacked barplots (percentage)
# -------------------------------------------------------------
stack_df <- conf_df %>%
...
cat("\n===============================================================\n")
cat("ATAC ILARIA PIPELINE — COLUMN-SCALED DOMINANCE\n")
cat("===============================================================\n\n")
