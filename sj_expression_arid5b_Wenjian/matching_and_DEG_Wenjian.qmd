...
write.csv(df_updated, "clean_ilarias_700_genes.csv", row.names = F)
...
Module scores are simply the average standardized expression of lineage-specific genes, transforming noisy gene-level data into stable, interpretable developmental programs.
...
```
#### STATISTICAL EXPLORATION AS OF JANUARY 17, 2026
###### Stage specific ARID
...
#### CUX1 and CUX2 expression
```{r}

# ============================================================
# CUX1 expression (FPKM) by genotype across B-cell development
# Prism-style bar plot with jittered points
# ============================================================
...
# Gene of interest
gene_id <- "ENSG00000257923"  # CUX1

# Genotype colors (Prism style)
geno_colors <- c(
    "TT" = "#000000",  # black
    "CT" = "#9e9e9e",  # gray
    "CC" = "#FFFFFF"   # white
...
# Prepare data: merge expression, genotype, and cell type info
plot_df <- expression_long %>%
    filter(gene == gene_id) %>%
    left_join(genotypes_clean, by = "sample_id") %>%
    left_join(dominant_tbl %>% select(sample = sample, top_ct), by = c("sample_id" = "sample")) %>%
    filter(!is.na(genotype_label), !is.na(expression), !is.na(top_ct)) %>%
    mutate(
        genotype_label = factor(genotype_label, levels = c("TT","CT","CC")),
        top_ct = factor(top_ct, levels = c("HSCMPP","EarlyLymphoid","ProB","PreB","B","GMP","pDC"))
    )

cat("Samples retained for", gene_id, ":", nrow(plot_df), "\n")

# Prism-style bar plot
cux1_barplot <- ggplot(plot_df, aes(x = genotype_label, y = expression, fill = genotype_label)) +
    ...
    labs(
        title = "CUX1 expression by rs7090445 genotype across B-cell development",
        y = "CUX1 mRNA (FPKM)",
        x = NULL,
        fill = "rs7090445\ngenotype"
    )
...
# ============================================================
# CUX2 expression (FPKM) by genotype across B-cell development
# Prism-style bar plot with jittered points
# ============================================================
...
# Prism-style bar plot
cux2_barplot <- ggplot(plot_df, aes(x = genotype_label, y = expression, fill = genotype_label)) +
    
    # Mean bar (Prism-style)
    stat_summary(
        fun = mean,
        geom = "col",
        width = 0.6,
    ...
    labs(
        title = "CUX2 expression by rs7090445 genotype across B-cell development",
        y = "CUX2 mRNA (FPKM)",
        x = NULL,
        fill = "rs7090445\ngenotype"
    )
...
##### SAVING ARID5B AND CUX1:

```{r}

# ============================================================
# Export FPKM data for ARID5B and CUX1 with genotype & cell type info
# Includes gene_symbol column
# ============================================================
...
gene_map <- tibble(
    gene = c("ENSG00000150347", "ENSG00000257923"),  # ARID5B and CUX1
    gene_symbol = c("ARID5B", "CUX1")
)

# Prepare combined data
export_df <- expression_long %>%
    filter(gene %in% gene_map$gene) %>%
    left_join(genotypes_clean, by = "sample_id") %>%
    left_join(dominant_tbl %>% select(sample = sample, top_ct), by = c("sample_id" = "sample")) %>%
    filter(!is.na(genotype_label), !is.na(expression), !is.na(top_ct)) %>%
    left_join(gene_map, by = "gene") %>%
    select(gene, gene_symbol, sample_id, expression, genotype, ancestry, genotype_label, top_ct)

# Check dimensions
cat("Rows in export_df:", nrow(export_df), "\n")

# Save to CSV
output_path <- "/Users/cojulian/Desktop/Tzu_projects/Matt/Clarissa/snp_cell_lines_public_datasets_project/sj_expression_arid5b_Wenjian/csv_for_figures_RIP_Jan_2026/ARID5B_CUX1_FPKM_by_genotype.csv"
write_csv(export_df, output_path)
