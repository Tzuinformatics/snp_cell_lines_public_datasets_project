...
```{r}
# ---- 4) Gene-level matrices: ALL coding peaks vs PROMOTER-only ----
suppressPackageStartupMessages({
    library(dplyr)
    library(purrr)
})
...
# ---- 4.3 Spain-derived ladder marker panels (PROMOTER / ENHANCER) ----
...
# ---- 5) Build Spain core state list (incluyendo PROM-only + panels) ----
spain_state <- list(
    # Sample-level metadata
...
cat("\n[Spain] Core state with gene_counts_spain_prom + Spain marker panels saved in:\n  ",
    out_path, "\n")
