...
df <- data.frame(
  ...
  donor.group = c(
    "1   D206    CC","2   D214    CC","3   D213    TC","4   D212    CC",
    "5   D201    TT","6   D200    TC","7   D212    CC","8   D215    TC",
    "9   D210    TT","10  D211    TT","11  D202    TC","12  D200    TC"
  )
)

# Split by whitespace and keep donor + group, then remove duplicates by donor
snp_group <- tidyr::separate(df, donor.group, into = c("drop","donor","group"), sep = "\\s+", extra = "drop") %>%
  dplyr::select(-drop) %>%
  dplyr::distinct(donor, .keep_all = TRUE)  # keep first occurrence of each donor
...
## Genotype colors (paper / Prism style)
geno_colors <- c(
    "TT" = "#000000",  # black
    "TC" = "#9e9e9e",  # gray
    "CC" = "#FFFFFF"   # white
...
## Prepare plotting data (no summarising yet)
plot_df <- arid5b_expr_bm %>%
    filter(cell_type %in% c("HSC","CLP","ProB","PreB","Immature_B")) %>%
    mutate(
        cell_type = factor(
            cell_type,
            levels = c("HSC","CLP","ProB","PreB","Immature_B")
        ),
        geno_call = factor(geno_call, levels = c("TT","TC","CC"))
    )

ggplot(plot_df, aes(x = geno_call, y = FPKM, fill = geno_call)) +
    ...
    labs(
        title = "ARID5B mRNA expression by rs709045 genotype across B-cell development",
        y = "ARID5B mRNA (FPKM)",
        x = NULL,
        fill = "rs709045\ngenotype"
    )
